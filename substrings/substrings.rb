...
# if substring key is included within the dictionary array
#   - include it in the subs hash, and increase it's count by 1

def substrings(str, dict_arr)
  dict_subs = Hash.new(0)
  (0...str.length).each do |i|
    (i...str.length).each do |j|
      key = str[i..j].downcase
      dict_subs[key] += 1 if dict_arr.include?(key)
    end
  end
  dict_subs.sort.to_h # specifically added sort, to match TOP's outcome
end
